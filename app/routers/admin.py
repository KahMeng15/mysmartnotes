...
def get_all_users(db: Session = Depends(get_db), admin: User = Depends(get_current_admin_user)):
    ...
    for u in users:
        notes_count = db.query(func.count(Note.id)).join(Resource).filter(Resource.user_id == u.id).scalar() or 0
        ...
        storage_bytes = db.query(func.sum(Resource.file_size)).filter(Resource.user_id == u.id).scalar() or 0
